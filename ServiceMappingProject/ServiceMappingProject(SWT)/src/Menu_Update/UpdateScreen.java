...
public class UpdateScreen extends Composite implements SelectionListener{
	...
	private Tree updateTree = null;
	private TreeColumn updateTreeColumn = null;
	private Group tableGroup = null;
	private Button selectAllTableItemButton = null;
	private Table updateTable = null;
	...
	public void createTree(){
		...
		updateTree = new Tree(treeGroup, SWT.BORDER | SWT.V_SCROLL | SWT.CHECK);	//트리 생성
		updateTree.setLinesVisible(true);	
		updateTree.setHeaderVisible(true);
		updateTreeColumn = new TreeColumn(updateTree, SWT.LEFT);
		updateTreeColumn.setWidth(170); 
		updateTreeColumn.setText("Companies / Services");
		updateTree.setLayoutData(WidgetProperty.setGridLayoutVerticalProperty(1, 450, SWT.FILL, true));
		...
		for(int index = 0; index < companyNumber; index++){	//회사 이름(테이블 명)
			TreeItem companyTreeItem = new TreeItem(updateTree, SWT.NONE);	//회사이름 트리 아이템 생성 
	...
	public void createTable(){
		...
		updateTable = new Table(tableGroup, SWT.BORDER | SWT.FULL_SELECTION | SWT.CHECK);	//테이블 생성
		updateTable.setLayoutData(WidgetProperty.setGridLayoutVerticalProperty(2, 440, SWT.FILL, true)); 

		String[] tableColumnList = {"Company", "Name", "DomainTask", "Stype"};	//테이블 컬럼 4개 생성
		for(int index = 0; index < tableColumnList.length; index++){
			tableColumn = new TableColumn(updateTable, SWT.LEFT); 
		...
		updateTable.setHeaderVisible(true);	//테이블 컬럼 보이기 
		updateTable.setLinesVisible(true);	//테이블 라인 보이기
	...
	public void createBriefButton(){
		viewBriefButton = new Button(this, SWT.PUSH); 	 
	...
	public void mountEventHandler(){
		updateTree.addSelectionListener(this);
	...
	public void widgetSelected(SelectionEvent e){
		...
		if(e.getSource() == selectAllTreeItemButton){	
			...
			TreeItem[] companyTreeItem = updateTree.getItems();
		...
		//▶버튼
			...
			TreeItem[] companyTreeItemList = updateTree.getItems();	//회사 트리 아이템 추출
			for(int i=0; i < companyTreeItemList.length; i++){
				...
				for(int j=0; j < serviceTreeItemList.length; j++){
					if(serviceTreeItemList[j].getChecked()){
						...
						TableItem tableItem = new TableItem(updateTable, SWT.NONE);	//테이블 아이템 생성
		...
		//▶▶자세히 보기 버튼
			...
			TableItem[] tableItem = updateTable.getItems();
			...
			UpdateDetailScreen updateDetailScreen = new UpdateDetailScreen();
			for(int i=0; i<tableItem.length; i++){
				if(tableItem[i].getChecked()){	//테이블 아이템이 체크 상태이면
					updateDetailScreen.createTabItem(tableItem[i].getText(1), tableItem[i].getText(0));	//서비스, 회사
				}
			}
			if(numberOfCheckedItem != 0){
				MainFrame.stackLayout.topControl = updateDetailScreen;
		...
		//트리
		else if(e.getSource() == updateTree){	
		...
		//초기화 버튼
			...
			TreeItem[] treeItem = updateTree.getItems();
